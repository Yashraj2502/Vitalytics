...
import com.example.vitalytics.Math.Biquad;
...
public class Cascade {
    ...
    // coefficients
    private com.example.vitalytics.Math.Biquad[] m_biquads;
    ...
    public com.example.vitalytics.Math.Biquad getBiquad(int index) {
    ...
    public Complex response(double normalizedFrequency) {
        ...
        for (int i = 0; i < m_numBiquads; i++) {
            com.example.vitalytics.Math.Biquad stage = m_biquads[i];
    ...
    public void setLayout(LayoutBase proto, int filterTypes) {
        ...
        m_biquads = new com.example.vitalytics.Math.Biquad[m_numBiquads];
        switch (filterTypes) {
            case DirectFormAbstract.DIRECT_FORM_I:
                m_states = new DirectFormAbstract[m_numBiquads];
                for (int i = 0; i < m_numBiquads; i++) {
                    m_states[i] = new DirectFormAbstract() {
                        @Override
                        public void reset() {
                        ...
                        @Override
                        public double process1(double in, com.example.vitalytics.Math.Biquad s) {
                            return 0;
                        }
        ...
        for (int i = 0; i < m_numBiquads; ++i) {
            ...
            m_biquads[i] = new com.example.vitalytics.Math.Biquad();
            m_biquads[i].setPoleZeroPair((BiquadPoleState) p);
        }
